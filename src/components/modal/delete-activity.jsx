...
export default function DeleteActivityModal({
  ...
  const handleDelete = async () => {
    ...
    if (!selectedActivitiyId) return;

    try {
      const response = await axios.delete(
        `${API_ENDPOINT}/api/activity/${selectedActivitiyId}/delete-activity`
      );

      if (response.status === 200) {
        toast.success(`Successfully deleted activity ${selectedActivityName}`, {
          style: {
      ...
      toast.error(
        `Failed to delete activity ${selectedActivityName}. Please try again later.`,
        {
          style: {
          ...
          >
            <div className="bg-white   px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              ...
              >
                ...
                <span className="capitalize">{selectedActivityName}</span>?
              </DialogTitle>
...
DeleteActivityModal.propTypes = {
  ...
  selectedActivitiyId: PropTypes.string.isRequired,
  selectedActivityName: PropTypes.string.isRequired,
};
